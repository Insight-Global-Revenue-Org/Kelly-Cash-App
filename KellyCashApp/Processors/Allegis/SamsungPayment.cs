...
{
    internal class SamsungPayment
    {
        private const int HeaderRow = 6;
        private const int FirstDataRow = 10;

        public static bool IsSamsungFormat(IXLWorksheet worksheet)
        {
                ...
                Dictionary<string, List<OirMatch>> openInvoiceMatches)
        {
            ...
            var outputRows = new List<SamsungOutputRow>();
            ...
            for (int row = FirstDataRow; row <= lastRow; row++)
            {
                ...
                if (matches.Any())
                {
                    ...
                    {
                        outputRows.Add(new SamsungOutputRow
                        {
                            ...
                            SamsungInvoice = microsoftInvoice
            ...
            for (int i = 0; i < outputRows.Count; i++)
            {
                ...
                worksheet.Cell(row, 10).Value = item.SamsungInvoice;
            ...
            string outputPath = GetUniqueOutputPath(downloadsPath, $"Samsung {processedDate} - {formattedTotal}.xlsx");

            workbook.SaveAs(outputPath);

            Analytics.LogRemittanceRun($"Samsung - {formattedTotal}");
    ...
    decimal targetAmount)
        ...
        private class SamsungOutputRow
        {
            ...
            public string SamsungInvoice { get; set; } = "";
