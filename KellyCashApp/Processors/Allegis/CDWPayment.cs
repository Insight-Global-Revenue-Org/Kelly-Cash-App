...
{
    internal class CDWPayment
    {
        private const int HeaderRow = 6;
        private const int FirstDataRow = 10;

        public static bool IsCDWFormat(IXLWorksheet worksheet)
        {
                ...
                Dictionary<string, List<OirMatch>> openInvoiceMatches)
        {
            ...
            var outputRows = new List<CDWOutputRow>();
            ...
            for (int row = FirstDataRow; row <= lastRow; row++)
            {
                ...
                if (matches.Any())
                {
                    ...
                    {
                        outputRows.Add(new CDWOutputRow
                        {
            ...
            for (int i = 0; i < outputRows.Count; i++)
            {
                ...
                worksheet.Cell(row, 10).Value = item.CDWInvoice;
            ...
            string outputPath = GetUniqueOutputPath(downloadsPath, $"CDW {processedDate} - {formattedTotal}.xlsx");

            workbook.SaveAs(outputPath);

            Analytics.LogRemittanceRun($"CDW - {formattedTotal}");
    ...
    decimal targetAmount)
        ...
        private class CDWOutputRow
        {
            ...
            public string CDWInvoice { get; set; } = "";
